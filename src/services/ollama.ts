...
export function hasPassiveRoleplayDrift(reply: string, userText: string, ongoingAdultContext = false) {
  if (!ongoingAdultContext && !explicitAdultContext.test(userText) && !intimateEuphemism.test(userText)) return false;
  const userAdvancesContact = /\bI\s+(?:start|continue|keep|kiss|touch|rub|press|move|slide|reach|grab|hold|pull|push|bite|lick|stroke|run|place|land|wrap|lean|turn|grind|rock)\w*\b/i.test(userText);
...
export function initiativeSafeHistory<T extends Pick<Message, "role" | "content">>(messages: T[]): T[] {
  ...
  return messages.filter((message) => {
    if (message.role === "user") {
      ...
      adultContext = adultContext || explicitAdultContext.test(message.content) || intimateEuphemism.test(message.content);
      return true;
    }
    return message.role !== "assistant" || !hasPassiveRoleplayDrift(message.content, latestUser, adultContext);
  });
}

export function groundedReciprocalInitiative(userText: string) {
  if (/\b(?:Natalie|sister|brother|mother|father|daughter|son)\b/i.test(userText)) return null;
  if (/\b(?:kiss|lips?|neck|collarbone)\b/i.test(userText)) {
    return "I slide one hand behind your neck and pull you into a deeper kiss, pressing myself firmly against you instead of waiting for you to do all the moving.";
  }
  if (/\b(?:grind\w*|rub\w*|bulge|hips?)\b/i.test(userText)) {
    return "I hook one leg around your hips and rock back against you, meeting each movement with one of my own as I pull you closer.";
  }
  if (/\b(?:hands?|touch|breast|chest|waist|sides?)\b/i.test(userText)) {
...
export function hasLatestActionOmission(reply: string, userText: string) {
  const clothingRequest = /\b(?:let'?s|get|take|pull|slide|remove|slip)\b[\s\S]{0,70}\b(?:pants|leggings|shorts|shirt|skirt|dress|underwear|clothes?)\b[\s\S]{0,35}\boff\b|\boff\b[\s\S]{0,35}\b(?:pants|leggings|shorts|shirt|skirt|dress|underwear|clothes?)\b/i.test(userText);
  if (!clothingRequest) return false;
  const addressesClothing = /\b(?:pants|leggings|shorts|shirt|skirt|dress|underwear|clothes?|waistband|hem|fabric|button|zipper|unfasten|unbutton|unzip)\b/i.test(reply);
  const replaysPriorBeat = /\b(?:pull me (?:onto|on) the bed|joining Natalie|our own exploration|sliding my hand into yours)\b/i.test(reply);
  return !addressesClothing || replaysPriorBeat;
}

export function hasInventedThirdPartyReaction(reply: string, userText: string, facts: EmilyFamilyFact[] = []) {
  if (!facts.some((fact) => new RegExp(`\\b${escapeRegex(fact.name)}\\b`, "i").test(`${userText}\n${reply}`))) return false;
  const reaction = /\b(?:moan(?:s|ed|ing)?|sigh(?:s|ed|ing)?|giggl(?:e|es|ed|ing)|arch(?:es|ed|ing)?|shiver(?:s|ed|ing)?|trembl(?:e|es|ed|ing)|relax(?:es|ed|ing)?|whimper(?:s|ed|ing)?|gasp(?:s|ed|ing)?)\b/gi;
  const stated = new Set((userText.match(reaction) ?? []).map((item) => item.toLowerCase().replace(/(?:s|ed|ing)$/, "")));
  return (reply.match(reaction) ?? []).some((item) => !stated.has(item.toLowerCase().replace(/(?:s|ed|ing)$/, "")));
}
...
export function hasUserParrotingDrift(reply: string, userText: string) {
  const userNarratesAction = /\bI\s+(?:start|continue|keep|kiss|touch|rub|press|move|slide|reach|grab|hold|pull|push|bite|lick|stroke|run|place|land|wrap|lean|turn|climb|walk|look)\w*\b/i.test(userText);
  if (!userNarratesAction || !/^\s*(?:You|Your)\b/i.test(reply)) return false;
  const emilyFirstPersonBeat = /\bI\s+(?:kiss|touch|rub|press|move|slide|reach|grab|hold|pull|push|bite|lick|stroke|wrap|lean|turn|arch|shiver|tremble|gasp|moan|whisper|say|breathe|smile|close|open|guide|meet|answer|return)\w*\b/i.test(reply);
  const emilyPhysicalResponse = /\b(?:shiver|trembl|gasp|moan|arch|pulse|tighten|melt|breath catch|body respond)\w*\b[\s\S]{0,35}\b(?:me|my body)\b|\b(?:me|my body)\b[\s\S]{0,35}\b(?:shiver|trembl|gasp|moan|arch|pulse|tighten|melt|breath catch|respond)\w*\b/i.test(reply);
  return !emilyFirstPersonBeat && !emilyPhysicalResponse;
}

export function nonParrotingHistory<T extends Pick<Message, "role" | "content">>(messages: T[]): T[] {
  let latestUser = "";
  return messages.filter((message) => {
    if (message.role === "user") {
      latestUser = message.content;
    ...
    return message.role !== "assistant" || !hasUserParrotingDrift(message.content, latestUser);
  });
}
...
export class OllamaClient {
  ...
  async chat(profile: Record<string, unknown>, history: Message[], memories: Memory[], style: StyleProfile | null, relationship: RelationshipSettings, userText: string, options: { photoWillBeGenerated?: boolean } = {}) {
    ...
    const messages: Array<Pick<Message, "role" | "content">> = [
      ...
      ...identitySafeHistory(initiativeSafeHistory(nonParrotingHistory(everydaySafeHistory(relationshipSafeHistory(spicySafeHistory(photoSafeHistory(languageSafeHistory(conversation.messages, userText), willGeneratePhoto), effectiveRelationship), familyFacts))))),
