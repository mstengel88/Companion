...
Register-ScheduledTask -TaskName "EmilyComfyUIWatchdog" -Action $action -Trigger $trigger -Settings $settings -Description "Restarts Emily's ComfyUI task after three failed health checks." -Force | Out-Null
Write-Host "Emily ComfyUI watchdog installed. It checks every five minutes." -ForegroundColor Green
